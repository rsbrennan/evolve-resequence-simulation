...
for k in {1..1} # Number of simulation replicates that you want to create.
do
    mkdir 'SimRep'$k
    cd 'SimRep'$k
...
for k in {1..1} # Set the number of simulation replicates that you want to create.
do
    echo $k
            ...
            -d FreqBound=0.0 \
            -d LowerPosBound=0 \
            -d UpperPosBound=19999999 \
            -d D=0.5 \
            -d OffspringSize=80000 \
            -d ParentSize=50 \
            -d Cutoff_Quantile=0.1 \
            -d "Quantile_label='10_percent'" \
            -d "OutPath='/users/r/b/rbrennan/evolve-resequence-simulation/simulations/NQTL10/'" \
